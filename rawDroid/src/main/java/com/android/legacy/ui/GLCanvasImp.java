...
public class GLCanvasImp implements GLCanvas
{
	...
	private static void putRectangle(float x, float y, float width, float height, float[] buffer, FloatBuffer pointer)
	{
		buffer[0] = x;
		buffer[1] = y;
		buffer[2] = x + width;
	...
	public void drawMesh(BasicTexture tex, int x, int y, int xyBuffer, int uvBuffer, int indexBuffer, int indexCount) throws
            DeadBitmapException
	{
		float alpha = mAlpha;
		if (!bindTexture(tex))
	...
	private void drawBoundTexture(BasicTexture texture, int x, int y, int width, int height) throws
            DeadBitmapException
	{
		...
		{
			setTextureCoords(0, 0, (texture.getWidth() - 0.5f) / texture.getTextureWidth(), (texture.getHeight() - 0.5f) / texture.getTextureHeight());
	...
	private void convertCoordinate(RectF source, RectF target, BasicTexture texture) throws
            DeadBitmapException
	{
	...
	public void drawMixed(BasicTexture from, BasicTexture to, float ratio, int x, int y, int w, int h) throws
            DeadBitmapException
	{
		drawMixed(from, to, ratio, x, y, w, h, mAlpha);
	}

	private boolean bindTexture(BasicTexture texture) throws DeadBitmapException
	{
	...
	private void drawMixed(BasicTexture from, BasicTexture to, float ratio, int x, int y, int width, int height, float alpha) throws
            DeadBitmapException
	{
		...
		// same size.
		Utils.Assert(from.getWidth() == to.getWidth() && from.getHeight() == to.getHeight());

		mGLState.setBlendEnabled(!from.isOpaque() || !to.isOpaque() || alpha < OPAQUE_ALPHA);
	...
	private static class GLState
	{
		...
		public void setColorMode(int color, float alpha)
		{
			...
			mGL.glColor4x(Math.round(((color >> 16) & 0xFF) * prealpha), Math.round(((color >> 8) & 0xFF) * prealpha), Math.round((color & 0xFF) * prealpha),
	...
	{
		String line = String.format("MESH:%d, TEX_OES:%d, TEX_RECT:%d, FILL_RECT:%d, LINE:%d", mCountDrawMesh, mCountTextureRect, mCountTextureOES,
