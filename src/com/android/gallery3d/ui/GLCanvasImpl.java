...
public class GLCanvasImpl implements GLCanvas
{
	...
	private void drawBoundTexture(BasicTexture texture, int x, int y, int width, int height)
	{
		...
		if (isMatrixRotatedOrFlipped(mMatrixValues))
		{
			...
			{
				setTextureCoords(1.0f / texture.getTextureWidth(), 1.0f / texture.getTextureHeight(), (texture.getWidth() - 1.0f) / texture.getTextureWidth(),
	...
	private void drawMixed(BasicTexture from, int toColor, float ratio, int x, int y, int width, int height, float alpha)
	{
		...
		setTextureColor(((toColor >>> 16) & 0xff) * colorScale, ((toColor >>> 8) & 0xff) * colorScale, (toColor & 0xff) * colorScale, combo);
	...
	private void setRenderTarget(RawTexture texture)
	{
		...
		else
		{
			...
			gl11ep.glFramebufferTexture2DOES(GL11ExtensionPack.GL_FRAMEBUFFER_OES, GL11ExtensionPack.GL_COLOR_ATTACHMENT0_OES, GL11.GL_TEXTURE_2D,
					texture.getId(), 0);
