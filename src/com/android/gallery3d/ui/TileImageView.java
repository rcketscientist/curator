...
import com.anthonymandra.framework.LongSparseArray;
...
public class TileImageView extends GLView
{
	...
	/*
	 * This is the tile state in the CPU side. Life of a Tile: ACTIVATED (initial state) --> IN_QUEUE - by queueForDecode() --> RECYCLED - by
	 * recycleTile() IN_QUEUE --> DECODING - by decodeTile() --> RECYCLED - by recycleTile) DECODING --> RECYCLING - by recycleTile() --> DECODED - by
	 * decodeTile() --> DECODE_FAIL - by decodeTile() RECYCLING --> RECYCLED - by decodeTile() DECODED --> ACTIVATED - (after the decoded bitmap is
	 * uploaded) DECODED --> RECYCLED - by recycleTile() DECODE_FAIL -> RECYCLED - by recycleTile() RECYCLED --> ACTIVATED - by obtainTile()
	 */
	...
	private class Tile extends UploadedTexture
	{
		...
		public Tile(int x, int y, int level)
		{
			mX = x;
			mY = y;
			Log.d(TAG, "mx=" + mX + ", my=" + mY + ", level= " + level);
			mTileLevel = level;
		}
		...
		boolean decode()
		{
			...
			try
			{
				mDecodedTile = DecodeUtils.ensureGLCompatibleBitmap(mModel.getTile(mTileLevel, mX, mY, TILE_SIZE, TILE_BORDER, sTilePool));
			}
			catch (Throwable t)
			{
