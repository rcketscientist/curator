...
class PositionController
{
	...
	public PositionController(Context context, Listener listener)
	{
		...
		mFilmScroller = new OverScroller(context, null /* default interpolator */, mFocusX, mFocusX); // API 11, not used anyway! , false /* noflywheel */);
	...
	private boolean setBoxSize(int i, int width, int height, boolean isViewSize)
	{
		...
		float ratio = (width > height) ? (float) b.mImageW / width : (float) b.mImageH / height;
...
// IMAGE_GAP + MAX(gapToSide(previous), gapToSide(current))
	...
	public boolean flingPage(int velocityX, int velocityY)
	{
		...
		mPageScroller.fling(p.mCurrentX, b.mCurrentY, velocityX, velocityY, mBoundLeft, mBoundRight, mBoundTop, mBoundBottom);
	...
	public boolean flingFilmX(int velocityX)
	{
		...
		if ((!mHasPrev && p.mCurrentX >= defaultX) || (!mHasNext && p.mCurrentX <= defaultX))
		{
			return false;
		}
	...
	public boolean inOpeningAnimation()
	{
		return (mPlatform.mAnimationKind == ANIM_KIND_OPENING && mPlatform.mAnimationStartTime != NO_ANIMATION)
				|| (mBoxes.get(0).mAnimationKind == ANIM_KIND_OPENING && mBoxes.get(0).mAnimationStartTime != NO_ANIMATION);
	}
	...
	private void layoutAndSetPosition()
	{
		...
		// This may not be ideal, could fire for prev/next, but shouldn't be a big deal.
		mListener.onScaleChanged(mBoxes.get(0).mCurrentScale);
		// dumpState();
	}
...
//	}
	...
	private abstract static class Animatable
	{
		...
		{
//			Log.d(TAG, "Animatable.advanceAnimation " + mAnimationKind);
	...
	private class Platform extends Animatable
	{
		...
		public boolean startSnapback()
		{
			...
			Box b = mBoxes.get(0);
			float scaleMin = mExtraScalingRange ? b.mScaleMin * SCALE_MIN_EXTRA : b.mScaleMin;
			float scaleMax = mExtraScalingRange ? b.mScaleMax * SCALE_MAX_EXTRA : b.mScaleMax;
		...
		protected boolean interpolate(float progress)
		{
			...
			else if (mAnimationKind == ANIM_KIND_FLING_X && com.anthonymandra.framework.Util.hasIceCreamSandwich())
			{
	...
	private class Box extends Animatable
	{
		...
		public boolean startSnapback()
		{
			...
			if (mAnimationKind == ANIM_KIND_SCROLL && mListener.isHoldingDown())
				return false;
			if (mAnimationKind == ANIM_KIND_DELETE && mListener.isHoldingDelete())
				return false;
...
//				Log.d(TAG, "mCurrentScale: " + mBoxes.get(0).mCurrentScale);
//				Log.d(TAG, "scale: " + scale);
//				Log.d(TAG, "mImageW: " + mBoxes.get(0).mImageW);
//				Log.d(TAG, "mImageH: " + mBoxes.get(0).mImageH);
				if (mFilmMode)
				{
					y = 0;
				}
...
//		int skipAnimCounter = 0;
		private boolean doAnimation(int targetY, float targetScale, int kind)
		{
			targetScale = clampScale(targetScale);
			if (mCurrentY == targetY && mCurrentScale == targetScale && kind != ANIM_KIND_CAPTURE)
			{
//				Log.d(TAG, String.valueOf(++skipAnimCounter));
				return false;
			}
