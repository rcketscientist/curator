...
public class PhotoView extends GLView
{
	...
	public interface Model extends TileImageView.Model
	{
		...
		// This amends the getScreenNail() method of TileImageView.ImageModel to get
	...
	public interface Listener
	{
		...
		public void onSingleTapConfirmed();// int x, int y); Don't need the point at the moment don't bother converting
...
//		mLoadingText = StringTexture.newInstance(context.getString(R.string.loading), DEFAULT_TEXT_SIZE, Color.WHITE);
		...
		public void handleMessage(Message message)
		{
			switch (message.what)
			{
				...
				case MSG_DELETE_ANIMATION_DONE:
				{
					...
					mListener.onDeleteImage((String) message.obj, message.arg1);
	...
	public void notifyDataChange(int[] fromIndex, int prevBound, int nextBound)
	{
		...
		// Move the boxes
		mPositionController.moveBox(fromIndex, mPrevBound < 0, mNextBound > 0, false/* mModel.isCamera(0) */, mSizes);
	...
	public void notifyImageChange(int index)
	{
		...
		notifyImageUpdate(index);
	}

	public void notifyImageUpdate(int index)
	{
	...
	// We created two methods to differentiate nail/full loads
//	public void notifyImageChange(int index)
//	{
//		if (index == 0)
//		{
...
//		Log.d(TAG, "compensation = " + mCompensation + ", CameraRelativeFrame = " + mCameraRelativeFrame + ", mCameraRect = " + mCameraRect);
	}
	...
	private class ScreenNailPicture implements Picture
	{
		...
		public void draw(GLCanvas canvas, Rect r)
		{
			...
			float filmRatio = mPositionController.getFilmRatio();
			boolean wantsCardEffect = CARD_EFFECT && mIndex > 0 && filmRatio != 1f && !mPictures.get(0).isCamera();
			boolean wantsOffsetEffect = OFFSET_EFFECT && mIsDeletable && filmRatio == 1f && r.centerY() != h / 2;
			int cx = wantsCardEffect ? (int) (interpolate(filmRatio, w / 2, r.centerX()) + 0.5f) : r.centerX();
			int cy = r.centerY();
			canvas.save(GLCanvas.SAVE_FLAG_MATRIX | GLCanvas.SAVE_FLAG_ALPHA);
			...
			// AJM: If you 'back' while tiles load this can be null.
//			if (mScreenNail == null)
//				return;
...
//				return (int) (delta + 0.5f);
		...
		private boolean flingImages(float velocityX, float velocityY)
		{
			...
			int escapeVelocity = (int) GalleryUtils.dpToPixel(SWIPE_ESCAPE_VELOCITY);
			int centerY = mPositionController.getPosition(mTouchBoxIndex).centerY();
			boolean fastEnough = (Math.abs(vy) > escapeVelocity) && (Math.abs(vy) > Math.abs(vx)) && ((vy > 0) == (centerY > getHeight() / 2));
			if (fastEnough)
			{
		...
		{
			MetaMedia item = mModel.getMediaItem(mTouchBoxIndex);
			if (item == null)
				return;
...
//				mHandler.sendEmptyMessage(MSG_UNDO_BAR_FULL_CAMERA);
	...
	private boolean swipeImages(float velocityX, float velocityY)
	{
		...
		if (!isMinimal && Math.abs(velocityY) > Math.abs(velocityX))
			if ((edges & PositionController.IMAGE_AT_TOP_EDGE) == 0 || (edges & PositionController.IMAGE_AT_BOTTOM_EDGE) == 0)
				return false;
