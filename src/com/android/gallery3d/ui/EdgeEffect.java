 ...
 * 
 * <p>
 * EdgeEffect is stateful. Custom widgets using EdgeEffect should create an instance for each edge that should show the effect, feed it input data
 * using the methods {@link #onAbsorb(int)}, {@link #onPull(float)}, and {@link #onRelease()}, and draw the effect using {@link #draw(Canvas)} in the
 * widget's overridden {@link android.view.View#draw(Canvas)} method. If {@link #isFinished()} returns false after drawing, the edge effect's
 * animation is not yet complete and the widget should schedule another drawing pass to continue the animation.
 * </p>
 * 
 * <p>
 * When drawing, widgets should draw their main content and child views first, usually by invoking <code>super.draw(canvas)</code> from an overridden
 * <code>draw</code> method. (This will invoke onDraw and dispatch drawing to child views as needed.) The edge effect may then be drawn on top of the
...
public class EdgeEffect
{
	...
	/**
	 * Reports if this EdgeEffect's animation is finished. If this method returns false after a call to {@link #draw(Canvas)} the host widget should
	 ...
	 * associated animation. The host view should always {@link android.view.View#invalidate()} after this and draw the results accordingly.
	 * 
	 * @param deltaDistance
	 *            Change in distance since the last call. Values may be 0 (no change) to 1.f (full length of the view) or negative values to express
	 *            change back toward the edge reached to initiate the effect.
	 */
	...
	/**
	 * Call when the object is released after being pulled. This will begin the "decay" phase of the effect. After calling this method the host view
	 * should {@link android.view.View#invalidate()} and thereby draw the results accordingly.
	 */
	 ...
	 * 
	 * <p>
	 * When using a {@link android.widget.Scroller} or {@link android.widget.OverScroller}, the method <code>getCurrVelocity</code> will provide a
	...
	/**
	 * Draw into the provided canvas. Assumes that the canvas has been rotated accordingly and the size has been set. The effect will be drawn the
	 * full width of X=0 to X=width, beginning from Y=0 and extending to some factor < 1.f of height.
	 * 
	 * @param canvas
	 *            Canvas to draw into
