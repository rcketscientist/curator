...
public class TileImageViewAdapter implements TileImageView.Model
{
	...
	public Bitmap getTile(int level, int x, int y, int tileSize, int borderSize, BitmapPool pool)
	{
		...
		{
			getTileApi11(pool, regionDecoder, bitmap, options, wantRegion);
		}
		else
		{
			getTileApi10(regionDecoder, bitmap, options, wantRegion);
		}
...
//		}
	...
	private void getTileApi11(BitmapPool pool, BitmapRegionDecoder regionDecoder, Bitmap bitmap, BitmapFactory.Options options, Rect wantRegion)
	{
		options.inBitmap = bitmap;
		...
		try
		{
			...
			synchronized (regionDecoder)
			{
				Log.d(TAG, (wantRegion.right <= 0) + ", " + (wantRegion.bottom <= 0) + ", " + (wantRegion.left >= regionDecoder.getWidth()) + ", " + (wantRegion.top >= regionDecoder.getHeight()));
				Log.d(TAG, wantRegion.right + "<= 0, " + wantRegion.bottom + "<= 0, " + wantRegion.left + ">= " + regionDecoder.getWidth() + ", " + wantRegion.top + ">= " + regionDecoder.getHeight());
				bitmap = regionDecoder.decodeRegion(wantRegion, options);
			}
		}
	...
	private void getTileApi10(BitmapRegionDecoder regionDecoder, Bitmap bitmap, BitmapFactory.Options options, Rect wantRegion)
	{
		// In CropImage, we may call the decodeRegion() concurrently.
		synchronized (regionDecoder)
		{
