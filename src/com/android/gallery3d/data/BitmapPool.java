...
public class BitmapPool
{
	...
	private Bitmap findCachedBitmap(JobContext jc, byte[] data, int offset, int length, Options options)
	{
		if (mOneSize)
			return getBitmap();
...
//	}
	...
	// TODO: Honeycomb
	public Bitmap decode(JobContext jc, byte[] data, int offset, int length, BitmapFactory.Options options)
	{
		if (Util.hasHoneycomb())
			return decodeApi11(jc, data, offset, length, options);
		else
			return decodeApi10(jc, data, offset, length, options);
	}

	private Bitmap decodeApi10(JobContext jc, byte[] data, int offset, int length, BitmapFactory.Options options)
	{
		if (options == null)
	...
	private Bitmap decodeApi11(JobContext jc, byte[] data, int offset, int length, BitmapFactory.Options options)
	{
		if (options == null)
			options = new BitmapFactory.Options();
		if (options.inSampleSize < 1)
