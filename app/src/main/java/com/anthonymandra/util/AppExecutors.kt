...
class AppExecutors(private val diskIO: Executor, private val networkIO: Executor, private val mainThread: Executor) {
    ...
    companion object {
        ...
        val DISK = INSTANCE.diskIO
        val NETWORK = INSTANCE.networkIO
        val MAIN = INSTANCE.mainThread

        /**
         * Utility method to run blocks on the dedicated disk thread, used for io/database work.
         */
        fun ioThread(f : () -> Unit) {
            DISK.execute(f)
        }
        /**
         * Utility method to run blocks on the dedicated network thread.
         */
