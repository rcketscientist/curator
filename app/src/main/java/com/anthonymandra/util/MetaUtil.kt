...
object MetaUtil {
	...
	/**
	 * Parse [imageInfo].uri and return a fully populated [ImageInfo]
	 */
	fun readMetadata(c: Context, imageInfo: ImageInfo): ImageInfo {
		val repo = DataRepository.getInstance(c)

		val uri = imageInfo.uri.toUri()
		var meta = readMeta(c, uri)
		meta = readXmp(c, uri, meta)

		imageInfo.altitude = getAltitude(meta)
		imageInfo.aperture = getAperture(meta)
		imageInfo.exposure = getExposure(meta)
		imageInfo.flash = getFlash(meta)
		imageInfo.focalLength = getFocalLength(meta)
		imageInfo.width = getImageWidth(meta)
		imageInfo.height = getImageHeight(meta)
		imageInfo.iso = getIso(meta)
		imageInfo.latitude = getLatitude(meta)
		imageInfo.longitude = getLongitude(meta)
		imageInfo.model = getModel(meta)

		imageInfo.orientation = getOrientation(meta)
		...
		imageInfo.whiteBalance = getWhiteBalance(meta)

		imageInfo.rating = getRating(meta)?.toFloat()
		getSubjectList(meta)?.let { imageInfo.subjectIds = repo.convertToSubjectIds(it) }
		imageInfo.label = getLabel(meta)
		imageInfo.lens = getLensModel(meta)
		imageInfo.driveMode = getDriveMode(meta)
		imageInfo.exposureMode = getExposureMode(meta)
		imageInfo.exposureProgram = getExposureProgram(meta)
	...
	/**
	 * Parses file info and adds the [file] under [parentMap] if possible to avoid db lookup.
	 */
	fun getImageFileInfo(c: Context,
								file: UsefulDocumentFile,
								parentMap: MutableMap<String, Long> = mutableMapOf()): MetadataEntity {
		return getImageFileInfo(c, file, parentMap)
	}
	...
	@WorkerThread
	/**
	 * Parses file info and adds the [file] parent to the database.
	 * *Optional* [parentMap] (uri to id), will be used first to avoid db lookup.
	 * *Optional* [parentId] will be used and no folder db lookup will occur.
	 */
	private fun getImageFileInfo(c: Context,
								file: UsefulDocumentFile,
								parentMap: MutableMap<String, Long> = mutableMapOf(),
								parentId: Long? = null): MetadataEntity {
		val repo = DataRepository.getInstance(c)
	...
	fun parseMetadata(context: Context, images: List<ImageInfo>) {
		...
		images.forEach {
			val metadata = MetaUtil.readMetadata(context, it)
