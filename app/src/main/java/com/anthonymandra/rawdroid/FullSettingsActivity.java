...
public class FullSettingsActivity extends PreferenceActivity {
	...
	public static class SettingsFragmentStorage extends PreferenceFragment {
		...
		public void onResume() {
			...
			Preference size = getPreferenceManager().findPreference(KEY_RecycleBinSize);
			if (size != null) {
				size.setOnPreferenceChangeListener((pref, maxSize) -> {
					RecycleBin.setMaxSize((Integer) maxSize * 1024 * 1024L);
					return true;
				});
			}
		...
		private void updateLicense(License.LicenseState state) {
			...
			switch (state) {
				case error:
					...
					break;
				case pro:
					license.setSummary(getActivity().getString(R.string.prefSummaryLicense));
					break;
				case demo:
					license.setSummary(getActivity().getString(R.string.buypro));
					setBuyButton();
					break;
				case modified_0x000:
				case modified_0x001:
