...
class TutorialActivity : GalleryActivity() {
	...
	override fun onPostCreate(savedInstanceState: Bundle?) {
		...
		dataRepo.insertParents(tutorialParent).subscribe()  // TODO: Should sequence these to be safe
		...
		val imageEntries = assets.mapIndexed { i, asset ->
			val output = File(tutorialDirectory, "$imageName${i + 1}.jpg")
			FileOutputStream(output).use {
				BitmapFactory.decodeResource(resources, asset).compress(Bitmap.CompressFormat.JPEG, 100, it)
			}

			val image = MetadataEntity()
			image.uri = output.toURI().toString()
		...
		dataRepo.insertImages(*imageEntries.toTypedArray())
				.subscribeBy { databaseIds.addAll(it) }
	...
	override fun onDestroy() {
		...
		// This should cascade
		dataRepo.deleteParents(tutorialParent)

//        dataRepo.deleteImage(*databaseIds.toLongArray())
	}
